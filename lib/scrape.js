...
async function loadHistory(chat, targetStartTs, log = console.log) {
  ...
  let limit = 100, prevCount = -1, stalls = 0, backoff = 1000;
  const MAX_STALLS = 8;
  const t0 = Date.now();
  ...
  console.log(`  ✓ Loaded ${prevCount < 0 ? 0 : prevCount} messages.`);
...
async function exportChat(chat, opts) {
  const { startTs = null, endTs = null, outDir, loadedCount = 0 } = opts;
  ...
  for (let i = 0; i < inRange.length; i++) {
    ...
    const who = oneline(await senderLabel(msg, chat));
