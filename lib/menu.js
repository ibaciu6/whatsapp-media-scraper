...
async function confirmAndExport(inquirer, ctx) {
  ...
  while (true) {
    ...
    if (action === 'preview') {
      ...
      const all = await group.fetchMessages({ limit: Math.max(99999, loaded) });
    ...
    const { saved, skippedExisting, total } =
      await scrape.exportChat(group, { startTs, endTs, outDir, loadedCount: loaded, ...scope });
    ...
    async function runBatch(list, noun) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        try {
          ...
          const r = await scrape.exportChat(chat, { startTs, endTs, outDir, loadedCount: loaded, ...scope });
          ok.push([name, r]);
